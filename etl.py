        ...
        if not os.path.exists(ZIP_DIR + f"/{date}/ncvoter_Statewide.txt.zst"):
            ...
            unzip_proc = subprocess.Popen(
                [
                    "unzip",
                    "-p",
                    ZIP_DIR + f"/{date}/ncvoter_Statewide.zip",
            ...
            zstd_proc = subprocess.Popen(
                ["zstd", "-z", "-o", ZIP_DIR + f"/{date}/ncvoter_Statewide.txt.zst"],
                stdin=iconv_proc.stdout,
            )
...
def create_whole(new_date: datetime.date):
    ...
    con.sql(
        f"""
    COPY (select * from whole_table order by county_id, precinct_abbrv, street_name)
